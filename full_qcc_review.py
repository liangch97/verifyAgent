...
def _run_review_and_deliver(contract: Path, company: str, draft_json: Path, stamp: str) -> None:
    ...
    try:
        ...
        raw_text = "\n".join(b.get("text", "") for b in loaded.get("ordered_blocks", []))
        # Single-track sub-agent review (replaces old dual-track llm_extract + merge)
        agent_result = agent_review.review_and_correct(rule_ext, raw_text)
        agent_status = agent_result.get("_agent_status", "failed")
        if agent_status == "ok":
            corrections = agent_result.get("_corrections") or []
            print(f"[agent_review] ok in {agent_result.get('_elapsed_s')}s, "
                  f"attempts={agent_result.get('_attempts')}, "
                  f"corrections={len(corrections)}", flush=True)
        else:
            agent_error = agent_result.get("_agent_error", "unknown")
            print(f"[agent_review] FAILED after {agent_result.get('_attempts')} attempts: {agent_error}", flush=True)
            try:
                fs_text(f"⚠️ 子 agent 字段复核失败（{agent_error}），本份报告字段仅基于规则脚本，请人工逐项复核。")
            except Exception:
                pass
    except Exception as e:
        print(f"[agent_review wrap warn] {e}", flush=True)
        traceback.print_exc()
        agent_status = "failed"
        agent_error = str(e)[:200]

    # Shape agent_result back into rule_ext form when successful → single source of truth.
    if agent_status == "ok" and isinstance(agent_result, dict):
        ag = agent_result
        final_ext = dict(rule_ext)
        for k in ("title","contract_no","contract_type","project_name","sign_date",
                  "confidentiality_summary","ip_clauses_summary","liability_summary"):
            if ag.get(k) is not None:
                final_ext[k] = ag[k]
        for p in ("party_a","party_b"):
            if isinstance(ag.get(p), dict):
                final_ext[p] = dict(ag[p])
        if ag.get("amount_text") or ag.get("amount_yuan") is not None:
            final_ext["amount"] = ag.get("amount_text") or (str(ag["amount_yuan"]) if ag.get("amount_yuan") is not None else "")
        dates = dict(final_ext.get("dates") or {}) if isinstance(final_ext.get("dates"), dict) else {}
        if ag.get("perform_start"): dates["perform_start"] = ag["perform_start"]
        if ag.get("perform_end"):   dates["perform_end"]   = ag["perform_end"]
        final_ext["dates"] = dates
        for k in ("payment_terms","contacts","attachments"):
            if isinstance(ag.get(k), list) and ag[k]:
                final_ext[k] = ag[k]
        if isinstance(ag.get("bank_account"), dict):
            final_ext["bank_account"] = ag["bank_account"]
        rule_ext_for_pdf = final_ext
        llm_ext_for_pdf = ag  # same data; merged_basic_info will agree with rule
    else:
        rule_ext_for_pdf = rule_ext
        llm_ext_for_pdf = {}
    ...
    try:
        render_admin_pdf(
            rule_extracted=rule_ext_for_pdf or {},
            llm_extracted=llm_ext_for_pdf or {},
